...
int main() {
  ...
  for (matrix_type<double>::reverse_iterator it = A.rbegin(); it != A.rend();
       ++it)
    cout << " " << *it;
  cout << endl;
  ...
  for (matrix_type<double>::const_reverse_iterator it = C.rbegin();
       it != C.rend(); ++it)
    cout << " " << *it;
  cout << endl;
  ...
  cout << "\nrow iteration" << endl;
  for (matrix_type<double>::diterator<0> it = A.dbegin<0>(); it != A.dend<0>();
       ++it)
    cout << ' ' << *it << endl;

  cout << "\ncolumn iteration" << endl;
  for (matrix_type<double>::diterator<1> it = A.dbegin<1>(); it != A.dend<1>();
       ++it)
    cout << ' ' << *it;
  cout << "\n\n";

  k = 0;
  for (matrix_type<double>::diterator<0> it1 = A.dbegin<0>();
       it1 != A.dend<0>(); ++it1) {
    cout << "row " << k++ << ":";
    for (matrix_type<double>::diterator<1> it2 = A.dbegin<1>(it1);
         it2 != A.dend<1>(it1); ++it2)
      cout << " " << *it2;
    cout << '\n';
  }
  ...
  cout << "\nrow iteration" << endl;
  for (matrix_type<double>::diterator<0> it = C.dbegin<0>(); it != C.dend<0>();
       ++it)
    cout << ' ' << *it << endl;

  cout << "\ncolumn iteration" << endl;
  for (matrix_type<double>::diterator<1> it = C.dbegin<1>(); it != C.dend<1>();
       ++it)
    cout << ' ' << *it;
  cout << "\n\n";

  k = 0;
  for (matrix_type<double>::diterator<0> it1 = C.dbegin<0>();
       it1 != C.dend<0>(); ++it1) {
    cout << "row " << k++ << ":";
    for (matrix_type<double>::diterator<1> it2 = C.dbegin<1>(it1);
         it2 != C.dend<1>(it1); ++it2)
      cout << " " << *it2;
    cout << '\n';
  }
